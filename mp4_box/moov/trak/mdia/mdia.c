...
int write_mdia_box(FILE *fp_mp4, char *p_buf, metadata_t *metadata, track_id_e track_id)
{
    ...
    /* 8~ is [box data] */
    buf_pos += write_mdhd_box(fp_mp4, p_buf, metadata, track_id);

    buf_pos += write_hdlr_box(fp_mp4, p_buf, track_id);

    buf_pos += write_minf_box(fp_mp4, p_buf, metadata, track_id);
    ...
    generate_box_header(buf_pos, "mdia", p_buf);
